...
export function useIntentClassification(
  ...
  /**
   * Server-side classification wrapper.
   * Uses fast local guards for obviously non-build/edit messages (STOP, meta-questions, etc.)
   * before calling the classify-intent edge function.
   */
  const classifyUserIntent = useCallback(
    async (prompt: string): Promise<{ intent: AgentIntent; questions?: any[] } | null> => {
      const text = prompt.trim().toLowerCase();

      // Ignore tiny / system-like prompts
      if (prompt.length < 15 || prompt.startsWith("🔧 AUTO-FIX") || prompt.startsWith("🔧")) {
        return null;
      }

      const hasHistory = messageCount > 0;
      const hasExistingCode =
        !!(sandpackFiles && Object.keys(sandpackFiles).length > 0) || !!(previewHtml && previewHtml.length > 0);
      const existingFileNames = sandpackFiles ? Object.keys(sandpackFiles) : [];

      // ── Negative / STOP intent — always chat, never build/edit ──
      const hasNegation =
        /\b(do not|don't|dont|stop|no more|leave it|leave this|don't change|dont change|do nothing|no further|no changes)\b/i.test(
          text,
        );
      if (hasNegation) {
        // Treat as pure chat; do not call server, do not trigger build/edit.
        return { intent: "chat" as AgentIntent };
      }

      // ── Meta-questions about the conversation — always chat ──
      const isMetaQuestion =
        /\b(what was my request|what did i ask|what did you build|what did you do|what have you done so far|what did you change)\b/i.test(
          text,
        );
      if (isMetaQuestion) {
        return { intent: "chat" as AgentIntent };
   ...
   * This is the first line of defense against misfires (STOP, questions, etc.).
   */
  const fastClassifyLocal = useCallback(
    (text: string): AgentIntent | null => {
      ...
      if (
        /^(hello|hi|hey|yo|good morning|good evening|good afternoon|what'?s up|how are you|hello there|hey phoenix|sup|hiya|evening|morning|howdy|greetings|hey there|hi there|heya)\b/i.test(
          t,
        )
      )
        return "chat";
      ...
      if (
        /^(how'?s it going|what are you doing|can you talk|are you there|what'?s new|tell me something|how have you been|what'?s happening)\b/i.test(
          t,
        )
      )
        return "chat";
      ...
      if (
        /\b(what can you do|what are your .*(skills|capabilities|features)|how does this work|your core skills)\b/i.test(
          t,
        )
      )
        return "chat";
      ...
      if (
        /\b(phase by phase|step by step|i'll give you|ill give you|one at a time|let me explain|first let me|i'll share|ill share|i'll provide|ill provide|wait for my|before you start)\b/i.test(
          t,
        )
      )
        return "chat";
      ...
      const isQuestionLike =
        /^(what|how|why|can you|could you|should|is it|tell me|explain|help me understand|describe|what's|how's|why's|when|where|who)\b/i.test(
          t,
        );
      if (isQuestionLike) return "chat";
      // Conversational phrases
      if (
        /^(thanks|thank you|got it|i see|okay so|i understand|that makes sense|cool|great|nice|awesome|perfect|no worries)/i.test(
          t,
        )
      )
        return "chat";
      // Asking about something (even without question mark)
      if (/\b(what does|how does|how do|what is|what are|can i|should i|is there|would it|will it)\b/i.test(t))
        return "chat";
      // Ambiguous help-seeking — always chat
      if (
        /\b(can you help|i need help|i'?m stuck|this is weird|why is this happening|can you check something)\b/i.test(t)
      )
        return "chat";
      ...
      if (
        /^(build|create|make|add|generate|implement|develop|set up|scaffold|wire up|produce|write code for)\b/i.test(t)
      )
        return "build";
      ...
      if (
        /\b(build|create|make|generate|design)\b/i.test(t) &&
        /\b(app|application|website|dashboard|landing page|portal|system|platform|page|form|module|component)\b/i.test(
          t,
        )
      ) {
        return "build";
      ...
      // NOTE: negation is already filtered above; this only fires for clean confirmations.
      if (/^(yes|go ahead|do it|build it|sounds good|sure|let's go|proceed)\b/i.test(t) && t.length < 100) {
        return "build";
      }

...
import { Zap, Send, FolderOpen, Trash2, Loader2, ArrowRight, Copy, Rocket, Database, Shield, Cloud, BarChart3, Globe, Lock, Users, Cpu, Eye, Wand2, MessageSquare, Layers, RefreshCw, Lightbulb, CheckCircle2, Sparkles, Server, FileCode, ShoppingCart, LayoutDashboard, PenTool, Briefcase, ChevronRight } from "lucide-react";
  ...
  const HOW_IT_WORKS = [
    {
      ...
      description: "Tell Phoenix what you want in plain English. Be as detailed or brief as you like.",
      icon: MessageSquare,
      gradient: "from-[hsl(210,100%,56%)] to-[hsl(210,100%,40%)]",
    },
    {
      step: "02",
      title: "Instant preview loads",
      description: "Your app renders in under 1 second. A live, interactive preview — not a mockup.",
      icon: Eye,
      gradient: "from-[hsl(265,89%,62%)] to-[hsl(265,89%,48%)]",
    },
    {
      step: "03",
      title: "AI customizes in real time",
      description: "Iterate with chat. The AI refines code, adds features, and fixes bugs automatically.",
      icon: Wand2,
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col overflow-y-auto">
        ...
        >
          ...
          <motion.div variants={fadeUp} custom={2} className="flex flex-wrap gap-2 justify-center">
            {suggestions.map((s) => (
              <button
                ...
                className="text-xs px-3.5 py-2 rounded-full border border-border text-muted-foreground hover:text-foreground hover:border-[hsl(var(--primary)/0.4)] hover:bg-secondary transition-all"
              >
                {s}
              </button>
            ))}
          ...
          <motion.div variants={fadeUp} custom={3} className="flex justify-center">
            <button
              ...
              className="flex items-center gap-1.5 text-xs text-muted-foreground hover:text-[hsl(var(--primary))] transition-colors px-3 py-1.5 rounded-full border border-border hover:border-[hsl(var(--primary)/0.3)]"
            >
              ...
              {projects.slice(0, 6).map((project) => {
                ...
                return (
                  <div key={project.id} onClick={() => onOpenProject(project.id)} className="group flex items-center gap-3 p-3 rounded-xl border border-border hover:border-[hsl(var(--primary)/0.3)] hover:bg-secondary/50 cursor-pointer transition-all">
                    ...
                    <button onClick={(e) => { e.stopPropagation(); cloneProject(project.id); }} className="opacity-0 group-hover:opacity-100 text-muted-foreground hover:text-[hsl(var(--primary))] transition-all p-1" title="Clone">
                      <Copy className="w-3.5 h-3.5" />
                    </button>
                    <button onClick={(e) => { e.stopPropagation(); deleteProject(project.id); }} className="opacity-0 group-hover:opacity-100 text-muted-foreground hover:text-destructive transition-all p-1" title="Delete">
        ...
        <div className="max-w-5xl mx-auto px-6 py-8 flex flex-wrap items-center justify-center gap-8 md:gap-16">
          {[
            ...
            <div key={stat.label} className="text-center">
              <p className="text-2xl md:text-3xl font-bold bg-gradient-to-r from-[hsl(var(--primary))] to-[hsl(var(--accent))] bg-clip-text text-transparent">
                {stat.value}
              </p>
        ...
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-80px" }} variants={stagger} className="max-w-5xl mx-auto">
          <motion.div variants={fadeUp} className="text-center mb-14">
            ...
            <p className="text-muted-foreground text-base mt-3 max-w-lg mx-auto">
              Not just a frontend builder. Every app gets a real backend — database, auth, functions, storage, and more.
            </p>
          </motion.div>
            ...
            {FULL_STACK_FEATURES.map((feat, i) => {
              ...
              return (
                <motion.div
                  ...
                  variants={fadeUp}
                  custom={i}
                  className="p-5 rounded-2xl border border-border bg-background/60 backdrop-blur-sm hover:border-[hsl(var(--accent)/0.3)] hover:bg-background/80 transition-all group"
                >
      ...
      <section className="px-6 py-20 max-w-5xl mx-auto w-full">
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-80px" }} variants={stagger}>
          <motion.div variants={fadeUp} className="text-center mb-14">
            ...
            <p className="text-muted-foreground text-base mt-3 max-w-lg mx-auto">
              Phoenix World generates production-ready apps — not templates, not mockups.
            </p>
          </motion.div>
            ...
            {SHOWCASE_APPS.map((app, i) => {
              ...
              return (
                <motion.div
                  ...
                  variants={fadeUp}
                  custom={i}
                  className="p-5 rounded-2xl border border-border bg-card hover:border-[hsl(var(--primary)/0.2)] transition-all group cursor-pointer"
                  onClick={() => { setInput(`Build me a ${app.label.toLowerCase()} app`); inputRef.current?.focus(); window.scrollTo({ top: 0, behavior: "smooth" }); }}
                >
                  ...
                  <p className="text-[10px] text-[hsl(var(--primary))] mt-3 opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-1">
      ...
      <section className="px-6 py-20 bg-card/30 relative overflow-hidden">
        ...
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-80px" }} variants={stagger} className="max-w-5xl mx-auto relative z-10">
          <motion.div variants={fadeUp} className="text-center mb-14">
            ...
            <p className="text-muted-foreground text-base mt-3 max-w-lg mx-auto">
              Multi-agent orchestration, auto-fix, and instant previews. This is a different kind of tool.
            </p>
          </motion.div>
            ...
            {WHY_DIFFERENT.map((item, i) => {
              ...
              return (
                <motion.div
                  ...
                  variants={fadeUp}
                  custom={i}
                  className="flex gap-4 p-5 rounded-2xl border border-border bg-background/60 backdrop-blur-sm hover:border-[hsl(var(--primary)/0.2)] transition-all"
                >
        ...
        <div className="max-w-4xl mx-auto px-6 py-12 flex flex-col md:flex-row items-center gap-6 text-center md:text-left">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground leading-relaxed max-w-2xl">
              Your apps run on secure, isolated edge functions with per-project databases. Row-level security policies protect every table. Auth, storage, and secrets management are built in — not bolted on.
            </p>
          </div>
          <div className="flex items-center gap-2 shrink-0">
            {["SOC 2", "RLS", "Edge"].map((badge) => (
              <span key={badge} className="text-[10px] font-bold uppercase tracking-wider px-3 py-1.5 rounded-full border border-[hsl(var(--ide-success)/0.3)] text-[hsl(var(--ide-success))] bg-[hsl(var(--ide-success)/0.05)]">
                {badge}
              </span>
            ))}
          </div>
      ...
      <footer className="px-6 py-8 text-center">
        ...
        <p className="text-xs text-muted-foreground">
          Built with Phoenix World • © {new Date().getFullYear()} All rights reserved.
        </p>
      </footer>
    </div>
  );

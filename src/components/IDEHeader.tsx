          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <button onClick={onRenameClick} className="flex items-center gap-2 px-2 py-1 rounded-lg hover:bg-secondary/60 transition-colors min-w-0 group">
                <img src="/logo.png" alt="Phoneix" className="w-5 h-5 rounded-md object-cover shrink-0" />
      ...
      <div className="flex items-center gap-1 flex-1 pr-1.5">
        {isPreview && (
          <>
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <button onClick={() => { const iframe = getPreviewIframe(); iframe?.contentWindow?.postMessage({ type: "history-forward" }, "*"); }} className="p-1 text-muted-foreground hover:text-foreground transition-colors rounded-md hover:bg-muted/50">
